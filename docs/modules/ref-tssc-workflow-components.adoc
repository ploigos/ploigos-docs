...
| Security Static Code Analysis
| The pipeline will perform static analysis on source code to catch defects and vulnerabilities as early in the development lifecycle as possible, prior to deploying and running the code. 
|
* Static Security Scanning Tool

| Linting Static Code Analysis
| Catch programatic and stylistic errors in code
|
* Static Code Linting
Tool
...
| Remove "WIP" from Merge Request
| This step is an indicator that the new code is of sufficient quality to be considered a candidate for merging into the main branch of the primary repository. Typically this step is done by the developer, and involves a change of the name/label and state of the merge request within the source control tool. 
|
* Source Control Tool

| Connect IDE to MR#-DEV Environment
| The developer's IDE should support (typically via plugins) the ability to connect directly to the development environment. 
|
...
| Commit Change to Idea Branch on Fork of Repository
| An idea branch (or feature branch) is a place for developer to perform work to complete a single feature. Forked repository allows developer without direct access to primary repo to implement contributions that can then be pulled into primary. 
|
* Source Control Tool
